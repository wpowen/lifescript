...
struct SoloSettlementView: View {
    ...
    private var relationshipShifts: [RelationshipShift] {
        ...
        return book.characters.compactMap { character in
            ...
            return RelationshipShift(
                ...
                detail: SoloLocalization.format("当前态度转向「%@」，说明这条因果线已经被你真正拨动了。", current.attitudeLabel)
            )
        }
    }
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 18) {
                        ...
                        if !keyMoves.isEmpty {
                            detailBlock(title: SoloLocalization.localized("本章关键落子"), tint: SoloTheme.gold) {
                                VStack(alignment: .leading, spacing: 12) {
                                    ForEach(Array(keyMoves.enumerated()), id: \.offset) { index, move in
                                        bulletRow(
                                            index: index + 1,
                                            title: move,
                                            detail: SoloLocalization.localized("这一步会在后续章节里继续发酵，它不是一段文本的结束，而是下一段因果的开端。")
                                        )
                                    }
                                }
                            }
                        ...
                        if shouldShowDestinyPanel {
                            detailBlock(title: SoloLocalization.localized("天命与代价"), tint: destinyTint) {
                                VStack(alignment: .leading, spacing: 12) {
                                    HStack(alignment: .firstTextBaseline) {
                                        ...
                                        Spacer()
                                        Text(SoloLocalization.format("天命值 %d", stats.destiny))
                                    ...
                                    if !visibleCosts.isEmpty || !visibleRewards.isEmpty || !visibleRiskHints.isEmpty {
                                        VStack(alignment: .leading, spacing: 8) {
                                            if !visibleCosts.isEmpty {
                                                bulletLine(title: SoloLocalization.localized("消耗"), items: visibleCosts, tint: SoloTheme.crimson)
                                            }
                                            if !visibleRewards.isEmpty {
                                                bulletLine(title: SoloLocalization.localized("得到"), items: visibleRewards, tint: SoloTheme.jade)
                                            }
                                            if !visibleRiskHints.isEmpty {
                                                bulletLine(title: SoloLocalization.localized("风险"), items: visibleRiskHints, tint: SoloTheme.gold)
                                            }
                                        }
                                    }
                                }
                        ...
                        if !relationshipShifts.isEmpty {
                            detailBlock(title: SoloLocalization.localized("人心偏转"), tint: SoloTheme.jade) {
                                VStack(alignment: .leading, spacing: 12) {
                        ...
                        if !resolvedChoices.isEmpty {
                            detailBlock(title: SoloLocalization.localized("你的选择余波"), tint: SoloTheme.warmInk) {
                        ...
                        if let hook = chapter.nextChapterHook {
                            section(title: SoloLocalization.localized("下一章因果回响"), content: hook)
                        }
                    }
            ...
            .soloStoryChrome(title: branding.settlementTitle, kicker: SoloLocalization.localized("余波"))
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    Button(SoloLocalization.localized("关闭")) { dismiss() }
    ...
    private var introBody: String {
        switch book.genre {
        case .cultivation:
            return SoloLocalization.localized("这一局不只是收尾。真正留下来的，是你把谁拉进了局里、把哪一步先落了下去，以及天机录为此替你烧掉了多少后手。")
        case .apocalypsePower:
            return SoloLocalization.localized("这一夜留下来的不只是数字，更是你把避难区的人心和生路分别推向了哪里。")
        case .suspenseSurvival:
            return SoloLocalization.localized("这一段封存下来的，不只是线索本身，还有你把真相拧向了哪一个方向。")
        case .businessWar:
            return SoloLocalization.localized("这一轮真正结算的，不只是输赢，而是你把牌桌上每个人的位置悄悄换成了什么。")
        case .urbanReversal:
            return SoloLocalization.localized("这一手翻过去以后，留下来的不只是结果，还有所有人重新看待你的角度。")
        }
    }
    ...
    private var destinyHeadline: String {
        ...
        if destinyDelta < 0 && darknessDelta > 0 {
            return SoloLocalization.localized("窥天得势，反噬也在逼近")
        }
        if destinyDelta < 0 {
            return SoloLocalization.localized("你为这一步烧掉了天命")
        }
        if destinyDelta > 0 {
            return SoloLocalization.localized("这一局替你回收了天命")
        }
        if darknessDelta > 0 {
            return SoloLocalization.localized("局面虽然推进，代价也留在了身上")
        }
        return SoloLocalization.localized("这一步把后续代价彻底点亮了")
    }

    private var destinyDetail: String {
        let destinyDelta = statChanges[.destiny] ?? 0
        let darknessDelta = statChanges[.darkness] ?? 0
        let destinyText = destinyDelta == 0 ? nil : SoloLocalization.format("天命 %@", formatted(delta: destinyDelta))
        let darknessText = darknessDelta == 0 ? nil : SoloLocalization.format("心魇 %@", formatted(delta: darknessDelta))

        let parts = [destinyText, darknessText].compactMap { $0 }
        if !parts.isEmpty {
            return parts.joined(separator: SoloLocalization.localized("，")) + SoloLocalization.localized("。真正危险的从来不是一次消耗，而是你为了赢这一局愿意连续烧掉多少未来。")
        }

        return SoloLocalization.localized("数值没有大幅跳动，不代表代价不存在；很多反噬会在下一章才把脸露出来。")
    }
